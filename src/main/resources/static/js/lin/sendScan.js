...
function buildgrid() {
    $("#table_list_2").jqGrid(
        {
            url: "/getFindWaybillByBranchJson",//获取数据的地址
...
let WaybillApp = new Vue({
    ...
    methods: {
        showWaybill:function(datas){
            ...
            $("#receiptStaff").val(datas.receiptStaff);
            $("#destinationBranch").val(datas.destinationBranch);

            let data ={
                'waybillId':datas.waybillId,
                'destination':datas.destination,
                'destinationBranch':datas.destinationBranch,
                'nextBranch':datas.nextBranch,
            }
            console.log(data)
            $.ajax({
                url:'getFindSendScanData',
                type:'post',
                dataType:'json',
                data:data,
                success:function(data){
                    ...
                    $("#nextBranch").val(list.nextBranch);
                }
            });
            console.log($("#nextBranch").val());
