...
function buildgrid() {
    $("#table_list_2").jqGrid(
        {
            url: "/getWaybillReceipt?waybillState=0",//获取数据的地址
            datatype: "json",//返回的数据类型
            mtype: "POST", //提交方式
            // postData: "waybillState="+1,
            ...
            colNames: ['运单号', '寄件网点', '目的地', '收件人', '收件人电话', '寄件时间', '寄件人', '寄件人电话', '运单状态', '收件员',  '货物数量',  '操作人员', '操作网点', '重量', '操作'],//显示字段名
            colModel: [
...
let aap = new Vue({
    ...
    methods: {
        showWaybill: function (datas) {
            console.log("数据为")
            console.log(datas)
            aap.waybillId = datas.waybillId;
            aap.weight = datas.weight;
            let data = {
